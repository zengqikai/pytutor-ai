...
| 指标 | 当前 | 目标 | 测量方法 |
|------|------|------|---------|
| RAG Recall@3 | 未知 | ≥ 0.70 | 构建 30 题 Ground Truth，检索结果里是否命中对应知识点 |
| RAG Recall@5 | 未知 | ≥ 0.85 | 同上 |
| RAG MRR | 0 | ≥ 0.60 | 第一个相关结果排名的倒数平均 |
| HNSW QPS 损失 | 0 | ≤ 10% | 调参后向量检索速度不能降超过 10% |
| Token 节省率 | 0 | ≥ 25% | 动态截断 + RAG 压缩后的 prompt token 对比 |

### B 的目标

| 指标 | 当前 | 目标 | 测量方法 |
|------|------|------|---------|
| 诊断精确匹配率 | 55% (20例) | ≥ 80% (50例) | 扩展 50 例测试集，diagnosis_id == expected |
| M1-M8 平均 F1 | 未测 | ≥ 0.80 | 每类分别统计 Precision + Recall → F1 |
| 误报率 | 未测 | ≤ 15% | 干净代码触发误区的比例 |
| 提示合适率 | 100%（自评） | ≥ 85%（人评） | 30 例人工标注：提示等级是否恰当 |

### C 的目标

| 指标 | 当前 | 目标 | 测量方法 |
|------|------|------|---------|
| 回复评分 Kappa | 0 | ≥ 0.70 | Multi-Judge 3 次 vs 人工 30 例，Cohen's Kappa |
| 弱项推荐命中 | 0 | ≥ 60% | 推荐"应复习的知识点"后，学生在该知识点续错的概率 |
| 推荐练习完成率 | 未测 | 比随机高 20% | 推荐题完成率 vs 非推荐题完成率 |
| 多模型成本降低 | 0 | ≥ 30% | 简单问题走 flash，复杂问题走 pro：总 API 费用对比 |
...
| A 至少 3/5 指标达标 | Recall@3、MRR、Token 节省中至少达 3 项 |
| B 至少 3/4 指标达标 | 精确匹配率、F1、误报率中至少达 3 项 |
| C 至少 3/4 指标达标 | Kappa、推荐完成率、成本降低中至少达 3 项 |
