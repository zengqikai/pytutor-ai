...
from app.models.profile import StudentProfile, StudentWeakness, LearningEvent
...
async def teacher_overview(
    ...
    hint_low = (await db.execute(
        select(sa_func.count()).select_from(StudentProfile).where(StudentProfile.hint_dependency == "low")
    )).scalar() or 0
    hint_med = (await db.execute(
        select(sa_func.count()).select_from(StudentProfile).where(StudentProfile.hint_dependency == "medium")
    )).scalar() or 0
    hint_high = (await db.execute(
        select(sa_func.count()).select_from(StudentProfile).where(StudentProfile.hint_dependency == "high")
    )).scalar() or 0
    ...
    misconception_stats = [{"code": code, "name": name, "count": count} for code, name, count in mc_rows]
    ...
    for name, email, sp in student_rows:
        ...
        if sp.recent_misconceptions:
            try: mc = json.loads(sp.recent_misconceptions)
            except: pass
        students.append({
            "name": name,
            "email": email,
